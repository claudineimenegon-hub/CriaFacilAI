...
const SYSTEM_INSTRUCTIONS = [
  ...
  'For contextual_lifestyle when the supplied affordances include wearable, human presence and interaction mode are required: include one realistic person and allocate at least one wearable unit to functionally valid body use. Do not require every wearable item to be worn simultaneously. Keep the other campaign roles product-led and never let people dominate all four proposals.',
  'When that Lifestyle contains both a primary wearable and a distinct multi-unit wearable, prefer allocating one unit of the second wearable to its own valid anatomical anchor when the pose can reveal it naturally. Keep every remaining unit exclusively scene-displayed or naturally occluded according to the exact allocation; never force all products onto the person.',
  'In that required wearable Lifestyle, campaignIdea, visualStory and scene must visibly describe the human use. A product-only, tabletop or still-life composition is invalid; surfaces and supports may appear only as secondary context.',
  'Use productSemantics.wearableItemIds as the exact allowlist for body-worn allocation and physicalPlacement. Never assign a non-wearable item to the body in a mixed inventory.',
  ...
  'For editorial_craft_detail and concept_campaign, propose scene, lighting, narrative and photography around canonical IDs only. Final item selection is enforced deterministically by the local V3 policy and overrides any conflicting selection in this draft.',
  'For concept_campaign, keep every omitted canonical ID absent rather than echoing it as decoration, silhouette, prop or transformed motif. If multiple independent units are selected by policy, keep them physically separate and never stack, interlace, entangle or fuse them.',
...
function visibilityInstructions(normalizedInput) {
  return [
    ...
    'CONTEXTUAL_LIFESTYLE is not a still life when the supplied affordances include wearable: require one realistic person and at least one body-worn canonical unit at its functionally valid semantic placement. Other wearables may remain displayed or intentionally occluded/out of frame when fully accounted for. For non-wearables, continue deciding contextually whether human interaction adds genuine value.',
    ...
    'EDITORIAL_CRAFT_DETAIL prioritizes distinctive craftsmanship, geometry, materials, texture and construction. The local deterministic policy will select exactly one canonical item using observed evidence richness; provide a genuine detail photograph and do not import details from omitted items.',
    ...
    'CONCEPT_CAMPAIGN selection is assigned by local deterministic policy: prefer one complete atomic relationship, otherwise use independent canonical units when inventory permits, and use one unit only for a single-unit global inventory. Use exact canonical quantities, omit every other ID, and never imitate an omitted product in props or decoration.',
